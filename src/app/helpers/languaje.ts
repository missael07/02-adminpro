...
export class lanjuage{
    ...
    public dashboardTitle = localStorage.getItem('lan') === 'ES' ? 'Bienvenido ' : 'Welcome ';
    public settings = localStorage.getItem('lan') === 'ES' ? 'Configuración perfil de ' : 'Settings for user ';
    ...
    public passwordField = localStorage.getItem('lan') === 'ES' ? 'Contraseña' : 'Password';
    public confirmPasswordField = localStorage.getItem('lan') === 'ES' ? 'Confirmar Contraseña' : 'Confirm Password';
    public emailField = localStorage.getItem('lan') === 'ES' ? 'Correo Electronico' : 'Email';
    public termsField = localStorage.getItem('lan') === 'ES' ? 'Acepto los Terminos y Condiciones' : 'Agree with Terms';
    public rememberMeField = localStorage.getItem('lan') === 'ES' ? 'Recordar Usuario' : 'Remember me';
    //#endregion Fields
    
    //#region Buttons
    public resetBtn = localStorage.getItem('lan') === 'ES' ? 'Recuperar' : 'Reset';
    public logOutBtn = localStorage.getItem('lan') === 'ES' ? 'Cerrar Sesión' : 'LogOut';
    public submitBtn = localStorage.getItem('lan') === 'ES' ? 'Guardar' : 'Submit';
    //#endregion Buttons

    //#region Modal Messages
    public saveMessage = localStorage.getItem('lan') === 'ES' ? 'Información guardada.' : 'Data saved.';
    public saveTitle = localStorage.getItem('lan') === 'ES' ? 'Guardado.' : 'Saved.';
    public warningMessage = localStorage.getItem('lan') === 'ES' ? 'Debes de cambiar tu foto de perfil, desde tu cuenta de google.' : 'You must change your profile picture from your GOOGLE ACCOUNT..';
    public warningTitle = localStorage.getItem('lan') === 'ES' ? 'Atención' : 'Attention';
    ...
    /* Back Errors */
    public googleError = localStorage.getItem('lan') === 'ES' ? 'Usuarios de Google no pueden editar su correo electronico .' : `Google user can't update his email.`;
    public emailPasswordError = localStorage.getItem('lan') === 'ES' ? 'Correo/Contraseña Invalidos.' : 'Invalid Email/Password.';
    public adminError = localStorage.getItem('lan') === 'ES' ? 'Error Inesperado, Contacte al Administrador.' : 'Unknown error, Contact to your administrator.';
    public emailExistsError = localStorage.getItem('lan') === 'ES' ? 'El correo electronico ya ha sido registrado.' : 'Email already was registered.';
    public fileExtensionError = localStorage.getItem('lan') === 'ES' ? 'Extensión no soportada.' : 'Extension unsupported.';
    public fileUnLoad = localStorage.getItem('lan') === 'ES' ? 'No se agrego ninguna imagen para cargar.' : 'There is not any image tu upload.';
    public fileError = localStorage.getItem('lan') === 'ES' ? 'Error al subir la imagen.' : 'An error occurred trying to upload the image.';
    public foundError = localStorage.getItem('lan') === 'ES' ? 'No se encontro información.' : 'No data found.'; 
    public validationField = localStorage.getItem('lan') === 'ES' ? 'Uno o más campos son requeridos.' : 'Required fields has an empty value.';       
